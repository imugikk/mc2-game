...
class Ingredient: SKSpriteNode, HandleContactEnter, HandleContactExit {
    ...
    init() {
        let texture = SKTexture(imageNamed: "POTIONR")
    ...
    func setupType(type: IngredientType){
        ...
        switch type {
        case .red:
            self.texture = SKTexture(imageNamed: "POTIONR")
            stringRepresentation = "R"
        case .blue:
            self.texture = SKTexture(imageNamed: "POTIONG")
            stringRepresentation = "B"
        case .yellow:
            self.texture = SKTexture(imageNamed: "POTIONY")
