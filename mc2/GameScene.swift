...
class GameScene: SKScene {
    let entityManager = GKEntityManager()
    ...
    override func didMove(to view: SKView) {
        ...
        let playerEntity = PlayerEntity(node: playerNode)

        let enemyNode = SKSpriteNode(imageNamed: "enemy")
        let enemyEntity = EnemyEntity(node: enemyNode, targetEntity: playerEntity, health: 100, agent: GKAgent2D())

        // Add entities to the entity manager
        entityManager.add([playerEntity, enemyEntity])

        // Add entity nodes to the scene
        addChild(playerNode)
        addChild(enemyNode)
    }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        // Update the entity manager and its entities
...
//    private var obstacleEntities: [GKAgent2D] = []
...
//
//        // Find all obstacle nodes in the scene and create obstacle entities
//        obstacleNodes = children.compactMap { $0 as? SKSpriteNode }.filter { $0.name == "obstacle" }
//        obstacleEntities = obstacleNodes.map { obstacleNode in
//            let obstacleEntity = GKAgent2D()
//            obstacleEntity.position = vector_float2(Float(obstacleNode.position.x), Float(obstacleNode.position.y))
//            obstacleEntity.radius = Float(obstacleNode.size.width) * 0.5 // Adjust the radius as needed
//            obstacleEntity.mass = 0.5 // Adjust the mass as needed
...
//            enemyEntity.component(ofType: EnemyComponent.self)?.update(deltaTime: currentTime - (lastUpdateTime ?? currentTime))
//        }
//
//
...
//        let enemyComponent = EnemyComponent(node: enemyNode, targetEntity: playerEntity, scene: self, health: 3, obstacleEntities: obstacleEntities)
...
//            case 2: // "D" key
//                playerComponent.node.position.x += 10 // Adjust the position as needed
//            default:
//                break
//            }
//        }
